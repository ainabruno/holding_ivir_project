...
import { isValidSourceUrl, pipelineStatusMessage, shouldOfferExtractionRetry } from "@/lib/adminPipeline";
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-4 text-slate-950 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        {jobStatus && <Alert className={jobStatus.startsWith("Erreur") ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}>{jobStatus.startsWith("Erreur") ? <AlertCircle className="h-4 w-4 text-red-600" /> : <CheckCircle className="h-4 w-4 text-emerald-600" />}<AlertDescription className={jobStatus.startsWith("Erreur") ? "text-red-800" : "text-emerald-800"}><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span>{jobStatus}</span>{shouldOfferExtractionRetry(jobStatus, runResult?.success) && <Button type="button" size="sm" variant="outline" onClick={handleTriggerExtraction} disabled={isLoading || frontendOnly} className="border-red-300 bg-white text-red-800 hover:bg-red-100">Relancer l’extraction</Button>}</div></AlertDescription></Alert>}
